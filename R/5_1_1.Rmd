...
ages <- 
  ...
  mutate(across(age_1960:age_2021, convert_pct, 0.1))
```
...
subsid_data <- 
  ...
  mutate(pct = convert_pct(subsid / subsid_total, 0.1)) |> 
  mutate(subsid = convert_number(subsid, 10))

can_subsid_pct <- convert_pct(can_subsid$subsid / 100, 0.1)
qc_subsid_pct <- convert_pct(qc_subsid$subsid / 100, 0.1)
```
