...
starts_maxima <- 
  ...
  mutate(value_trend = convert_number(value_trend, 1)) |> 
  filter(max)
...
start_market_min_max <- 
  ...
  filter(year >= 1992) |> 
  summarize(
    owner_max = convert_number(max(value_trend[market == "Homeowner"]), 1),
    owner_year = year[value_trend == max(value_trend[market == "Homeowner"])],
    condo_max = convert_number(max(value_trend[market == "Condo"]), 1),
    condo_year = year[value_trend == max(value_trend[market == "Condo"])],
    rental_max = convert_number(max(value_trend[market == "Rental"]), 1),
